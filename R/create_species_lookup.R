#' Pair NESPP3, SVSPP3, Species_itis
#'
...
#' @param skipType Character string. species Type to ignore in join. For example maybe only want to join SVSPP with ITIS. Same as \code{speciesType}
...
create_species_lookup <- function(
  channel,
  species = NULL,
  speciesType = "NESPP3",
  skipType = NULL
  ...
  if (is.null(species)) {
    message("No species codes were selected")
    return(NA)
  }
  missing <- list()
  ...
  if (tolower(speciesType) == "nespp3") {
    ...
    sql1 <- paste0(
      "select distinct NESPP3, NESPP4, NAFOSPP, SVSPP from NEFSC_GARFO.cfdbs_cfspp where ",
      toupper(speciesType),
      " in (",
      speciesToFind,
      ") order by NESPP4"
    ...
    missing[["cfdbs.cfspp"]] <- tibble::enframe(
      setdiff(species, unique(speciesTable1$NESPP3)),
      name = NULL,
      value = "NESPP3"
    )
    ...
    missing_nespp4s <- setdiff(
      unique(speciesTable1$NESPP4),
      unique(speciesTable2$NESPP4)
    )
    ...
    missing[["cfdbs.species_itis_ne"]] <- speciesTable1 %>%
      ...
      dplyr::select(.data$NESPP3)

    sciNames <- paste0(
      "'",
      unique(speciesTable2$SCIENTIFIC_NAME),
      "'",
      collapse = ","
    )

    # join 2 tables by NESPP4
    speciesTable <- dplyr::inner_join(
      speciesTable1,
      speciesTable2,
      by = "NESPP4"
    ) %>%
      dplyr::select(-.data$NESPP4) %>%
      dplyr::distinct()
    ...
    if (!is.null(skipType)) {
      ...
      sql3 <- paste0(
        "select distinct COMNAME, SCINAME, SVSPP from svdbs.svspecies_list where SCINAME in (",
        sciNames,
        ") order by SVSPP"
      )
      speciesTable3 <- DBI::dbGetQuery(channel, sql3) %>%
        dplyr::rename(SCIENTIFIC_NAME = .data$SCINAME) %>%
        dplyr::mutate(SVSPPsv = .data$SVSPP) %>%
        dplyr::select(-.data$SVSPP)

      # compare species passed vs species found to get missing species
      missing[["svdbs.svsspecies_list"]] <- setdiff(
        unique(speciesTable2$SCIENTIFIC_NAME),
        unique(speciesTable3$SCIENTIFIC_NAME)
      )
    ...
    sql1 <- paste0(
      "select distinct COMNAME, SCINAME, SVSPP from svdbs.svspecies_list where SVSPP in (",
      speciesToFind,
      ") order by SVSPP"
    )
    ...
    # compare species passed vs species found to get list of missing species
    missing[["svspp.svspecies_list"]] <- tibble::enframe(
      setdiff(species, unique(speciesTable1$SVSPP)),
      name = NULL,
      value = "SVSPP"
    )
    ...
    sql2 <- paste0(
      "select distinct COMMON_NAME, SCIENTIFIC_NAME, SPECIES_ITIS, NESPP4 from NEFSC_GARFO.cfdbs_species_itis_ne where SCIENTIFIC_NAME in (",
      sciNames,
      ") and NESPP4_FLAG = 1 order by NESPP4"
    )
    speciesTable2 <- DBI::dbGetQuery(channel, sql2) %>%
      dplyr::mutate(
        SCIENTIFIC_NAME = gsub("^\\s+|\\s+$", "", .data$SCIENTIFIC_NAME)
      ) %>%
      dplyr::mutate(COMMON_NAME = gsub(", ", ",", .data$COMMON_NAME))

    # compare species passed vs species found to get missing species
    missing[["cfdbs.species_itis_ne"]] <- setdiff(
      unique(speciesTable1$SCIENTIFIC_NAME),
      unique(speciesTable2$SCIENTIFIC_NAME)
    )
    ...
    if (!is.null(skipType)) {
      ...
      # if any string exceeds 1000 items the sql query will fail.
      # So split into smaller calls
      res <- split_sql_query_svspp(
        channel,
        speciesTable2$NESPP4,
        maxItems = 900
      )
      sql3 <- res$sql
      speciesTable3 <- res$speciesTable

      # compare species in table 1 vs species found in table 2 to get missing species
      missing_nespp4s <- setdiff(
        unique(speciesTable2$NESPP4),
        unique(speciesTable3$NESPP4)
      )
      missing_nespp3s <- unique(substring(missing_nespp4s, 1, 3))
      # select nespp3s missing from species_itis_ne
    ...
    sql1 <- paste0(
      "select distinct COMMON_NAME, SCIENTIFIC_NAME, SPECIES_ITIS, NESPP4 from NEFSC_GARFO.cfdbs_species_itis_ne where SPECIES_ITIS in (",
      speciesToFind,
      ") and NESPP4_FLAG = 1 order by NESPP4"
    )
    speciesTable1 <- DBI::dbGetQuery(channel, sql1) %>%
    ...
    # compare species passed vs species found to get list of missing species
    missing[["cfdbs.species_itis_ne"]] <- tibble::enframe(
      setdiff(species, unique(speciesTable1$SPECIES_ITIS)),
      name = NULL,
      value = "SPECIES_ITIS"
    )
